...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!authorSlug) {
      return NextResponse.json({ error: "Author slug not provided" }, {
        status: 400,
      });
    }
    if (!subscriberEmail) {
      return NextResponse.json({ error: "Subscriber email not provided" }, {
        status: 400,
      });
    }
    ...
    return NextResponse.json({ message: "Unsubscribe request processed." }, {
      status: 200,
    });
  } catch (error) {
