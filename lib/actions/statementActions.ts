...
export async function getDrafts({
  ...
  let drafts = db
    .selectFrom('draft')
    .leftJoin('draft as previousDraft', 'draft.parentStatementId', 'previousDraft.statementId')
    ...
    .select(({ eb }) => [
      ...
      'statementUrl.slug',
      jsonArrayFrom(
        eb.selectFrom('annotation').selectAll().whereRef('annotation.draftId', '=', 'draft.id')
      ).as('annotations'),
      jsonArrayFrom(
        eb
  ...
  const draftsObject = objects.reduce((acc: Record<string, Statement>, draft) => {
...
export async function getFullThread(threadId: string): Promise<DraftWithUser[]> {
...
export async function getPublishedStatement(statementSlug: string): Promise<DraftWithUser | null> {
...
export async function createDraft({
  ...
  const { statementId: returnedStatementId } = await db.transaction().execute(async tx => {
  ...
  if (returnedStatementId) {
    redirect(`/[userSlug]/${returnedStatementId}?version=${versionNumber}&edit=true`);
  } else {
