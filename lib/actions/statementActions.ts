...
export async function getPublishedOrLatest(
  ...
  const publishedDraftVersion = drafts.filter(draft => draft.publishedAt !== null)[0]
    ?.versionNumber;
  ...
  if (drafts.length > 0 && userIsCollaborator) {
    ...
    console.log('publishedDraftVersion', publishedDraftVersion);
...
export async function createStatement({
  ...
  if (returnedSlug) {
    redirect(`/${creatorSlug}/${returnedSlug}/1?edit=true`);
  } else {
