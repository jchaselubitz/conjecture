...
export async function getDraftsByStatementSlug(
  ...
  const draftPackage = await db.transaction().execute(async (tx) => {
    ...
    const annotations = await tx.selectFrom("annotation")
      .selectAll().where(
        "annotation.draftId",
        "in",
        draftIds,
      ).orderBy("annotation.createdAt", "desc").execute();

    const annotationIds = annotations.map((annotation) => annotation.id);

    let comments: BaseComment[] = [];
    if (annotationIds.length > 0) {
      comments = await tx.selectFrom("comment").select(({ eb }) => [
        "comment.id",
        "comment.content",
        "comment.createdAt",
        "comment.updatedAt",
        ...
        "in",
        annotationIds,
      ).orderBy("comment.createdAt", "desc").execute();
    }
    ...
    const profiles = await tx.selectFrom("profile").selectAll().where(
      "profile.id",
      "in",
  ...
  const draftWithAnnotations = drafts.map((draft) => ({
    ...
    annotations: annotations.map((a) => ({
      ...a,
      comments: comments.filter((c) => c.annotationId === a.id).map((c) => ({
        ...c,
