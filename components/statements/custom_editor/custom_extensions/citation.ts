...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    citation: {
      insertCitation: (options?: { id: string }) => ReturnType;
      updateCitation: (options: { citationId: string }) => ReturnType;
      ...
      updateCitation:
        ...
        ({ tr, state, dispatch }) => {
          ...
          doc.descendants((node, pos) => {
            if (node.type.name === this.name && node.attrs.citationId === options.citationId) {
              nodePos = pos;
              return false;
            }
            return true;
          ...
          if (nodePos === -1) {
            console.warn(`No citation node found with ID: ${options.citationId}`);
            return false;
          }
    ...
    const plugins = [
      ...
      new Plugin({
        ...
        appendTransaction: (transactions, oldState, newState) => {
          ...
          const pendingDeletions = this.editor.storage.citationDeletion?.pendingDeletions;
