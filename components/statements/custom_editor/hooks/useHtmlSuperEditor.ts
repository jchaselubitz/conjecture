...
export const useHtmlSuperEditor = ({
  ...
  const { setEditor, setUpdatedStatement, updatedStatement } =
    useStatementContext();
  ...
  const editor = useEditor({
    ...
    onCreate: ({ editor }) => {
      ...
      const annotationMarks = getMarks(editor, [
        "annotationHighlight",
      ]) as MarkInfo[]; // Use MarkInfo[] type
    ...
    onUpdate: ({ editor, transaction }) => {
      if (transaction.docChanged) {
        ...
        const newStatement = {
          ...updatedStatement,
          draft: { ...updatedStatement.draft, content: newContent },
        };
        startTransition(() => {
      ...
      if (!isNaN(start) && !isNaN(end) && start < end && start >= 0) {
        ...
        try {
          ...
          setTimeout(() => {
            ...
            if (newParams.get("location") === location) {
