...
interface HTMLTextAnnotatorProps {
  ...
  adjustable?: boolean; // Whether annotations can be adjusted with handles
    ...
    annotations.forEach((annotation) => {
      ...
      if (startNode && endNode) {
        try {
          ...
          // Then add handles if this annotation is selected and adjustable
          if (isSelected && adjustable) {
            ...
            if (markInDom) {
              ...
              markInDom.style.position = "relative";
          ...
          try {
            ...
            rangeNodes.forEach((nodeInfo) => {
              ...
              // Add dataset properties for consistency with main approach
              mark.dataset.start = annotation.start.toString();
              mark.dataset.end = annotation.end.toString();
              mark.dataset.draftId = annotation.draftId
                ? annotation.draftId.toString()
                : "";
              mark.dataset.selected = isSelected.toString();

              nodeRange.surroundContents(mark);

              // Add handles to the last segment if this is selected and adjustable
              if (
                isSelected &&
                adjustable &&
                nodeInfo === rangeNodes[rangeNodes.length - 1]
              ) {
                // Add handles code similar to main approach
                const startHandle = document.createElement("div");
                const endHandle = document.createElement("div");
                startHandle.className = "handle-container start";
                endHandle.className = "handle-container end";
