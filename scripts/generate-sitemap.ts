...
// Prefer explicit override for CI/one-off runs, then NEXT_PUBLIC_SITE_URL, then canonical www
const BASE_URL =
  process.env.SITEMAP_BASE_URL || process.env.NEXT_PUBLIC_SITE_URL || 'https://www.conject.io';
...
async function generateSitemap() {
  ...
  try {
    ...
    console.warn('Database connection failed, generating sitemap with static pages only');
  ...
  const sitemapPath = join(process.cwd(), 'public', 'static-sitemap.xml');
